...
import { checkPluginDependencies } from '../cordova';
...
export async function updateWindows(config: Config): Promise<void> {
...
async function writePluginsJson(
  ...
  const assemblies = await findDotNetAssemblies(plugins);
  const pluginsJsonPath = resolve(
    config.windows.nativeTargetDirAbs,
    'capacitor.plugins.json',
  );

  await writeJSON(pluginsJsonPath, assemblies, { spaces: '\t' });
}

async function findDotNetAssemblies(
  plugins: Plugin[],
): Promise<PluginsJsonEntry[]> {
  const entries: PluginsJsonEntry[] = [];

  const windowsPlugins: Plugin[] = plugins.filter(
    p => p.windows && getPluginType(p, platform) === PluginType.Core,
  ) as Plugin[];

  for (const plugin of windowsPlugins) {
    entries.push({
      pkg: plugin.id,
      assembly: plugin.windows?.assembly ?? '',
    });
  }
...
async function installNugetPackages(config: Config, plugins: Plugin[]) {
  await runTask(`Updating Windows native dependencies`, () => {
    return Promise.all([
...
async function updateNugetConfig(config: Config, plugins: Plugin[]) {
  ...
  if (!capacitorWindowsPath) {
    fatal(
      `Unable to find ${c.strong(
        'node_modules/@ionic-enterprise/capacitor-windows',
      )}.\n` +
  ...
  const nugetConfigPath = resolve(
    config.windows.nativeProjectDirAbs,
    'nuget.config',
  );
  ...
  await Promise.all(
    plugins.map(async plugin => {
      ...
      const value = relative(
        config.windows.nativeProjectDirAbs,
        join(await realpath(plugin.rootPath), 'windows'),
      );
...
const alreadyRegistered = (packageSources: any, key: string, value: string) => {
  return !!packageSources[0].add.find(
...
async function registerNugetDependencies(config: Config, plugins: Plugin[]) {
  Promise.all(
    plugins.map(async plugin => {
      ...
      runCommand('dotnet', ['add', 'package', pkgName], {
